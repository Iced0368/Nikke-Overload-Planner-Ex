...
- 상태별 권장 행동 제공
  ...
  - 수치 재설정
  - 모듈 잠금 / 락키 잠금 조합
- 입력한 모듈 예산 안에서 목표 달성 확률 최대화 정책 계산
...
모듈 예산을 입력한 뒤 예산 확률 계산 버튼을 누르면, 해당 예산 안에서 목표 달성 확률이 최대가 되는 정책을 계산합니다.

- 현재 시작 상태 기준 최대 달성 확률
- 해당 예산에서의 추천 행동과 잠금 방식
- 예산을 늘릴 때 달성 확률이 어떻게 변하는지 곡선으로 확인 가능

이 계산은 락키를 직접 제한하지 않으며, 같은 달성 확률이라면 기대 락키 소모가 더 적은 정책을 우선합니다.
